...
const getListBooks = function (palavraChave) {
    let keyword = palavraChave;
    let listBooks = [];
    let erro = true;

    if (typeof (keyword) != 'undefined') {
        //Percorre o array de livros para validar a palavraChave
        livros.forEach (key => {
            key.books.forEach(item => {
                //Localiza a palavra chave dentro do array
                if (item.title.toLowerCase().indexOf(keyword.toLowerCase()) >= 0) 
                {
                    let book = {};
                    book.nome = item.title;
                    book.descricao = item.subtitle;
                    book.preco = item.price;
                    book.imagem = item.image;
    ...
    let booksJSON = {};
    booksJSON.total = listBooks.length
    booksJSON.books = listBooks
    if (erro)            
        return false;
    else
        return booksJSON;
